%macro SUMTAB_ODSv2(data=_LAST_,     /* Input data set                      */
              ...
              stats=N {INT} MEAN {+1} STD {+1} STDMEAN {+1} 
                    MEDIAN {+1} Q1 {+1} Q3 {+1} MIN MAX) ;
                                                 /* PROC UNIVARIATE   */
                                                 /* key words list    */
%***********************************************************************
*
...
***********************************************************************
*	rename input dataset to __core1									  *
**********************************************************************;
proc contents data=&data noprint out=__core1 ;
  run ;
...
***********************************************************************
*	create _null_ dataset which assigns the number of observations	  * 
*	(nobs) in the input data set to the local macro	variable _nobs_   *
**********************************************************************;
data _null_ ;
  set __core1(obs=1) ;
...
*----------------------------------------------------------------------;
***********************************************************************
*	looks for the string '<' and executes a do while loop to assign   * 
*	values to cnt2, ncb, and cb. for instances where you need to use  *
*   a "check box variable" (i.e. race), you would want to obtain the  *
*	denominator (total) value from the race variable, which will be   *
*	the first variable in the list (i.e. <RACE BLACK WHITE OTHER>).	  *
*	This value is assigned to cb (check box).
*	
*	piece:
*	cnt1: external do loop counter
*	cnt2: counter internal to do loop
*	vopt0: 
*	i:
*	j:
*	k:
*	cb: check box variable list 
*	ncb: number of check box variables <RACE BLACK WHITE> would be 3  *
**********************************************************************;
%local piece cnt1 cnt2 vopt0 i j k cb ncb ;

%let ncb=0 ;
%let cnt1 = %index(&var,%str(<)) ;
%put CNT1 before DO: &cnt1;
%do %while(&cnt1) ;
  %let cnt2 = %eval(%index(&var,%str(>)) + 1) ; *<RACE BLACK WHITE> = 19;
  %let ncb = %eval(&ncb + 1) ; *ncb = 1;
  %let cb = &cb %substr(&var,&cnt1,%index(%substr(&var,&cnt1),%str(>))); *<race black white>, as typed;
  %if &cnt1=1 and %length(&var)=%eval(&cnt2-1) %then %do;
    *var is converted to #x, with x being the number of categorical lists 
  	(i.e.- <race b w> would be #1 and <sex m f> would be #2);
	%let var = #&ncb ;  
/*	%put VAR1: &var;*/
  %end;

  %else %if &cnt1=1 %then %do;
	%let var = #&ncb %substr(&var,&cnt2) ;		 
/*	%put VAR1: &var;*/
  %end;
  %else %if %length(&var)>%eval(&cnt2-1) %then %do;
    %let var = %substr(&var,1,%eval(&cnt1-1)) #&ncb %substr(&var,&cnt2) ;
	%put VAR1: &var;
%end;
  %else %do;
	%let var = %substr(&var,1,%eval(&cnt1-1)) #&ncb ; 
	%put VAR1: &var;
  %end;

  %let cnt1 = %index(&var,%str(<)) ; *switch do loop off (0); 
%end ;
...
%do %while (&go) ; *do at least once (default go == 1);
  %let piece = %scan(&cb,&cnt1,%str(<>)) ;
  %put PIECE_IN_DO: &piece;
  %if %length(&piece)>0 %then %do ;
    %let check = 0 ;
    %let cnt2 = %eval(&cnt2 + 1) ; *cnt2 ==1;
    %local cb&cnt2 ;
    %let cb&cnt2 = &piece ;
	%put CB&CNT2: &&cb&cnt2;
	%put PIECE>0: &piece;
  ...
  %end;
  %else %do;
	%let check = 1 ; *when piece !> 0, check==1, then loop again (go==1) and set go == 0;
	%put GO_ELSE: &go;
  %end;
  %let cnt1 = %eval(&cnt1 + 1) ; *cnt1==4: loops through 3 times. Will always be 4 if piece >0;
  %put CNT1: &cnt1;
%end ;
...
*parse  the categorical variable list and assign  values in the list to 
 variables associated with the corresponding "element"
 of the list, starting at 0, and incrementing sequentially;
%local cbvars ;
%put NCB: &ncb;

%do i = 1 %to &ncb ;
  %let cbvars = &cbvars %upcase(&&cb&i) ;*&&cb&i == 'race black white';
  %put CBI: &&cb&i; 
  %put cbvars is: &cbvars ;

  %let cnt1 = 1 ;
  %let piece = %scan(&&cb&i,&cnt1,%str( )) ; *assign 'race' to piece;
  %put PIECE_CBI: &piece;
  %do %while (&piece~=) ;
  %put CBI_DO_WHILE: &&cb&i;
    %local cb&i._%eval(&cnt1-1) ; *cnt1-1 == 0;
    %let cb&i._%eval(&cnt1 - 1) = %upcase(&piece) ; *<race black white>1_0 = 'race';
    %let cnt1 = %eval(&cnt1 + 1) ; *increment cnt1;
	*scroll through list of variables in &&cb&i NOTE: space (' ') is delimiter;
...
********************************************************
*	assign variable labels to var&cnt1 and variable "type" to
*	vopt&cnt1. 
*	i: count var for categorical variables 
*	   (i.e.- <race b w> would resolve to i == 1
*	cnt1: count var corresponding to number of variables in
*	      the var list (i.e.- "<race b w> two three", would resolve
*		  to cnt1==3
*	cnt2: count var used to initially read in the first element
*		  in the var list
**********************************************************;
%let cnt1=0 ;
%let cnt2=1 ;
...
**********************************************;
%put PIECE2: &piece;
%put VOPTCNT: &&vopt&cnt1;
%put VARCNT1: &&var&cnt1;
%put I: &i;
%put CNT1_PICE: &cnt1;
%put CNT2_PICE: &cnt1;
**********************************************;
...
%do i = 1 %to &nvar ;
  %if &&vopt&i ~= B %then %do;
	%let varlst = &varlst &&var&i ;
	%put VOPTI_INDO: &&vopt&i;
  %end;
%end ;
...
! 	__core2 is the dataset from proc contents procedure
*----------------------------------------------------------------------;
...
data _null_;
  length label $200 ;
  set __core2(keep=format label formatd formatl length type name) ; *<- TYPE IS A VAR FROM CONTENTS PROC;
  name = upcase(name); *changed for V9 compatibility;
  length format2 $20 ;
  ...
  *******************************************************;
  if format = ' ' then formatl = 32;
  else if format = '$' then formatl = 31;
  *******************************************************;
  select (trim(name)) ;
    when (upcase("&across")) do ;
    ...
    %do i = 1 %to &nvar ;
      when (upcase("&&var&i")) do ;
        ...
        call symput("tvar&i",trim(left(type))) ; *<- TVAR CREATED HERE FROM TYPE^^^^^^^^^^^^;
...
%if &ncb>0 %then %do ;
  ...
  %if %quote(&across)~= & %quote(&pageby&by)~= %then %do ;
    ...
    proc freq data=&cbindat ;
      tables %if %quote(&pageby)~= %then %do ;
			   %do i = 1 %to &npby ;
                 %str(&&pby&i *)
			   %end ;
			 %end ;
    ...
    proc sort data=&cbindat out=__corex3 nodupkey ;
	  by &pageby &by ;
      run ;

	data __corex2 ;
	  merge __corex2 __corex3(in=in) ;
	  by &pageby &by ;
	  if in ;
	  run ;
...
%put across is: &across;
%put pageby is: &pageby;
%put by is: &by;
...
%end ;
 %put PAGEBY and BY: &pageby AND &by;
...
proc sort data=&out;
	by descending labtst;
run;

/*proc datasets library=work nolist ;*/
/*  delete __core1 ;*/
/*  run ;*/
%put COUNTLEVELS MACRO CALLED;
%countlevels(&data,&stats,&by,&pageby)
%*---------------------------------------------------------------------*
! Print out a report:
*----------------------------------------------------------------------;
%if &print=Y %then %do ;
...
*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
*	Code for determining ods page layout 
*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&;
%local odsrows;
%let odsrows = 4;
%global tmplt;
%let tmplt = 30;
/*%if &naclev >= 2 && &naclev < 4 %then %do;*/
/*	%let odsrows = 4;*/
/*	%put ODSROWS: &odsrows;*/
/*	%put NACLEV: &naclev;*/
/*%end;*/
/*%else %if &naclev > 4 %then %do; */
/*	%let odsrows = 3;*/
/*	%put ODSROWS: &odsrows;*/
/*	%put NACLEV: &naclev;*
/*%end;*/
/*%if &naclev <1 %then %let odsrows = 4;*/
/*data out2;*/
/*	set &out (where=(&by= 'Total Bilirubin' | &by= 'Sodium' | &by= 'ALT' /*| &by= 'Albumin' /*| &by= 'Urea'*/*/
/*						/*| &by= 'LDH' | &by= 'Calcium'*/));
/*run;*/
/*%put OUT: &out;*/
/*%do i = 1 %to &nlabtests;*/ /**/
...
ods pdf /*file = 'c:\temp\sumtab_test.pdf'*/ uniform contents=no bookmarkgen=no style=ods_labtst2;
ods layout start columns = &odsrows;


  proc report data=&out nowd missing split="*" 
		style(report)={font_size=8pt font_face='Times New Roman, Helvetica, Helv' cellpadding=1pt cellspacing=0 rules=ALL}       
		style(header)={font_size=8pt font_face='Times New Roman, Helvetica, Helv' font_weight=extra_light}       
		style(column)={font_size=8pt font_face='Times New Roman, Helvetica, Helv' just=left cellpadding=0};

    column 	&dummy run __var__ variable _fmtd &by __stat__
...
/*    %end ;*/
    ...
    %else %do ;
      %if %upcase(&varlbl)~=_NONE_ %then %do ;
        define dummy / order width=&indent spacing=0 "*" ;
        define run     / order order=internal noprint spacing=0 ;
        define __var__ / order order=internal noprint spacing=0 ;
        define variable/ order order=internal noprint spacing=0 ;
      ...
      %else %do ;
        define run     / order order=internal noprint spacing=0 ;
        define __var__ / order order=internal noprint spacing=0 ;
        define variable/ order order=internal noprint spacing=0 style(column) = {};
      %end ;
    %end ;
    %do i = 1 %to &nby ;
      %if %index(&&by&i,NOPRINT)=1 %then %do ;
      ...
      %else %do ;
        ...
        %end ;
        %else %do ;
		  define _fmtd / order order=formatted noprint f=$report_order.;
          define &&by&i / order ;
        %end ;
      %end ;
    %end ;
...
ods layout end;
/*ods pdf close;*/
/*%end; *end number of times proc report is to run;*/
  %*-------------------------------------------------------------------*
...
*&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&77;
data lclmcrovars ;
  set sashelp.vmacro (where=(scope = 'LOCAL'));
;
run;

proc sort data=lclmcrovars;
	by name;
run;
...
/*%let bullshit  = 12;*/
/*%local morebullshit;*/
/*%let morebullshit = where the hell is it? ;*/
/**/
/*data temp ;*/
/*  set sashelp.vmacro;*/
/*  run;*/
